...
public class MediaController extends FrameLayout implements IMediaController {
    ...
    private ImageButton mediaFullSrceen;
    private boolean mUseFastForward;
    public  OnCallListener mOnCallListener;
    //全部设置的是android 内置的图片 与布局和 id 可以换成自己写的xml 设置id
  /*  // 播放的图片
...
*/
    ...
    public void  imnage( View CoverView){
    ...
    protected View makeControllerView() {
        return ((LayoutInflater) mContext
                .getSystemService(Context.LAYOUT_INFLATER_SERVICE)).inflate(R.layout.layout_video_press, this);
      /*  return ((LayoutInflater) mContext
    ...
    private void initControllerView(View v) {
        ...
        mediaFullSrceen = (ImageButton) v.findViewById(R.id.media_full_screen);
        mediaFullSrceen.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                //监听方法
                if (mOnCallListener != null) {
                    mOnCallListener.OnCallListener(true);
                }
            }
        });
        ...
        mCurrentTime = (TextView) v.findViewById(R.id.tv_start_time);
    }
    ...
    private void updatePausePlay() {
        if (mRoot == null || mPauseButton == null)
            return;
        if (mPlayer.isPlaying())
            mPauseButton.setImageResource(R.mipmap.suspend);
        else
            mPauseButton.setImageResource(R.mipmap.viedio_zanting);
    }
        ...
        /*    if (mAnchor != null && mAnchor.getWindowToken() != null) {
            ...
            if (mFromXml) {
                ...
                if (mAnchor != null) {
                  ...
                  //  int[] location = new int[2];
                  /*  mAnchor.getLocationOnScreen(location);
                    int popupWidth = mAnchor.getMeasuredWidth();
                    int popupHeight = mAnchor.getMeasuredHeight();
                    mWindow.showAtLocation(mAnchor, Gravity.NO_GRAVITY, (location[0] + mAnchor.getWidth() / 2) - popupWidth / 2, location[1] - popupHeight);*/

                    mWindow.showAsDropDown(mTextView);
                  //  mWindow.showAtLocation(mAnchor, Gravity.CENTER, location[0], location[1]-mWindow.getHeight());
                 ...
                 //   mWindow.showAsDropDown(mCoverView);

                  /*  mWindow.showAtLocation(mAnchor, Gravity.TOP,
                            anchorRect.right, anchorRect.right);*/

                  //  Log.e("666",anchorRect.left+"$$$$"+anchorRect.right+"^^^^"+anchorRect.top+"******"+anchorRect.bottom+"&&&&&");
                   ...
                   /* mWindow.showAtLocation(mRoot, Gravity.BOTTOM,
                            anchorRect.right, anchorRect.right);*/
                    mWindow.showAsDropDown(mTextView);
                    Log.e("33333",anchorRect.left+"$$$$"+anchorRect.right+"^^^^"+anchorRect.top+"******"+anchorRect.bottom+"&&&&&");
                }
            }
            mShowing = true;
    ...
    public void  xinashi(View view){
        this.mTextView = view;
    }
    private View mTextView;

    public interface OnCallListener{
        void OnCallListener(boolean b);
    }
    public void setmOnCallListener(OnCallListener mOnCallListener) {
        this.mOnCallListener = mOnCallListener;
    }
    public ImageButton getMediaFullSrceen() {
        return mediaFullSrceen;
    }
}
