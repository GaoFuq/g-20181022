...
public class PickerView extends View {
    ...
    private void init() {
        ...
        mPaint.setColor(getResources().getColor(R.color.pop_choose_edu_black));
    }
    ...
    private void drawText(Canvas canvas, int position, int direction) {
        ...
        // 字体大小与透明度
        if (position==0){
            mPaint.setColor(getResources().getColor(R.color.pop_choose_address_yollow));
        }else {
            mPaint.setColor(getResources().getColor(R.color.pop_choose_edu_black));
        }
...
//        mPaint.setTextSize(size);
        ...
        canvas.drawText(mDataList.get(mCurrentSelected + direction * position),
                x, y, mPaint);
        mPaint.setColor(getResources().getColor(R.color.pop_choose_edu_black_line));//画文字
        canvas.drawLine(0,y+mDistance/3,mViewWidth,y+mDistance/3,mPaint);//画分割线
    }
