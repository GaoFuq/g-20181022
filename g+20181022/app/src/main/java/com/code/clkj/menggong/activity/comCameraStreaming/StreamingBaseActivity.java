...
public abstract class StreamingBaseActivity extends Activity implements
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mLogTextView = (TextView) findViewById(R.id.log_info1);
        mStatusTextView = (TextView) findViewById(R.id.streamingStatus1);
        mStatView = (TextView) findViewById(R.id.stream_status1);
...
//        int inputType = intent.getIntExtra(INPUT_TYPE, -1);
        try {
            ...
            // mProfile.setPublishUrl(inputText.substring(Config.EXTRA_PUBLISH_URL_PREFIX.length()));
        } catch (URISyntaxException e) {
            e.printStackTrace();
        }
       /* if (inputText.startsWith(Config.EXTRA_PUBLISH_URL_PREFIX)) {
            // publish url

        }*/
   /*     else
        if (inputText.startsWith(Config.EXTRA_PUBLISH_JSON_PREFIX)) {
            try {
                JSONObject  mJSONObject = new JSONObject(inputText.substring(Config.EXTRA_PUBLISH_JSON_PREFIX.length()));
